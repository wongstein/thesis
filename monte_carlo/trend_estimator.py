...
class TrendEstimator():
    ...
    def __init__(self, s_of_t_dict):
        ...
        self.holtwinters_object = holtwinters.HoltWintersEstimator(self.s_des_t, 2, len(self.s_des_t)/2)
    ...
    def make_s_of_t_matrix(self, s_of_t_dict):
        ...
        x = 0
        key_counter = -1
        self.last_day_training = datetime.date(2013, 1, 1) #stupidly early
...
def test_all_10():
    ...
    prediction = my_estimator.predict_forecasted_s_t(2, 15)
...
def test():
    test_input = {datetime.date(2014, 1, 1): {datetime.date(2014, 1, 3): 2, datetime.date(2014, 1, 1): 2, datetime.date(2014, 1, 2): 0}}
